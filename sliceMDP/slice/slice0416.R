...
repeat{ 
  ...
  m_all <- c(m_all,length(which(m!=0)))
  ...
  for (id2 in 1:n){
    ...
    { Ncy[id2] <- min(which(sum_w>1-u[id2]))###correct
      Ncy0[id2] <- max(which(sum_w<1-u[id2]))#not work
    } else {
      Ncy[id2] <- n
      Ncy0[id2] <- n
    }
  ...
  Nc_all<-c(Nc_all,Nc)
  Nc0 <- max(Ncy0)#new number of groups
  Nc_all0<-c(Nc_all0,Nc0)
  u_star<-min(u)
  Nc1<-max(which(sum_w<1-u_star))
  Nc2<-min(which(sum_w>1-u_star))#not work
  Nc_all1<-c(Nc_all1,Nc1)
  Nc_all2<-c(Nc_all2,Nc2)
  #E.update delta######################################################################
