...
repeat{ 
  ...
  mu <- NULL#matrix with column for components and row for iteration.
  lambda <- NULL
  for (j in 1:Nc)
  {
    if (m[j]!=0){
      ksi[j] <- mean(y[oc[[j]]])  
      lambda1 <- a0+m[j]/2
      lambda2 <- b0+crossprod(y[oc[[j]]]-ksi[j])/2+(k0*m[j]*(ksi[j]-mu0)^2)/((k0+m[j])*2)
      mu1 <- (k0*mu0+m[j]*ksi[j])/(k0+m[j])
      mu2 <- k0+m[j]
      lambda[j] <- rgamma(1,shape=lambda1,rate=lambda2)#1/sd
      mu[j] <- rnorm(1,mean=mu1,sd=1/sqrt(mu2*lambda[j]))
    } else {
      lambda[j]<-rgamma(1,shape=a0,rate=b0)
      mu[j]<-rnorm(1,mean=mu0,sd=1/sqrt(k0*lambda[j]))
    }
  }
  theta[,1:Nc]<-rbind(mu,1/lambda)#results from last iteration
  #C.update v##########################################
  alpha<-NULL
  beta<-NULL
  for (j in 1:Nc)
  ...
  for (ie in 1:n){
    for (je in (1:Nc)){
      if (w[je]>u[ie]){
        p[ie,je]<-dnorm(y[ie],mean=theta[1,je],sd=sqrt(theta[2,je]))
      }else{
        p[ie,je]<-0
      }
    }
