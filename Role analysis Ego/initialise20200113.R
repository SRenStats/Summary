init.egoergm<-function(ego.terms, G, p.ego.terms=NULL)
  ...
  if (is.null(p.ego.terms))
    ...
    if (G>1)
      ...
      label <- initial.clusters$cluster
    ...
    # for (i in 1:N)
    #   for (g in 1:G)
    #     lambda[i,g]<-1/(sqrt(t(group.theta[g,]-theta[i,])%*%(group.theta[g,]-theta[i,]))+tol) # nugget to avoid zero, distance of every ego-network with group.theta
    # lambda<-lambda/apply(lambda,1,sum) # normalise lambda
    print("Finished kmeans initialisation")
    }
  return(list(theta=theta, group.theta=group.theta, label=label, G=G))
  }
